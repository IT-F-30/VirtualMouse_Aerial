...
MediaPipe が公開しているモデル（.tflite 形式）を ONNX 形式に変換し、それを Rust で読み込んでハンドトラッキングを実現してください
そのために
TFLite モデルを ONNX 形式に変換する python コード(MediaPipe のモデル変換用)
と
Rust で ONNX モデルを読み込み、推論を実行するコードを書いてください
